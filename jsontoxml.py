...
def convert_dict(obj, ids, parent, attr_type):
    ...
    for key, val in obj.items():
        # bon
        if key == 'attributes':
            addline('<attributes%s></attributes>' % (make_attrstring(val)))
        else:
...
def convert_list(items, ids, parent, attr_type):
    ...
    for i, item in enumerate(items):
        ...
        if type(item) in (int, float, long, str, unicode):
            ...
            if not attr_type:
                addline('%s' % (convert_dict(item, ids, parent, attr_type)))
            else:
                addline('%s' % (convert_dict(item, ids, parent, attr_type)))
        elif type(item) in (list, set, tuple) or isinstance(item, collections.Iterable):
            if not attr_type:
                addline('%s' % (convert_list(item, ids, 'item', attr_type)))
            else:
                addline('%s' % (convert_list(item, ids, 'item', attr_type)))
        elif item == None:
...
def dicttoxml(obj, root=True, custom_root='root', ids=False, attr_type=True):
    ...
    xml = ''.join(output).encode('utf-8')
    print xml
    xml = xml.replace('</children>', '')
    xml = xml.replace('<children>', '')
    xml = xml.replace('><attributes', '')
    xml = xml.replace('</attributes>', '')
    return xml
