...
class LoadConfig(object):
    ...
    def load_configFile(self, config_file, isFile=True, displayonly=False,
        ...
        self.config['apic'] = {'user_name': "admin",
                               'password': "ins3965!",
                               'ip_address': "172.31.160.110",
                               'use_https': False}
...
class TestCheckForAllTheJsonConfigs(unittest.TestCase):
    ...
    def test_Map_RDO_NOT_TOUCH_v1_policies(self):
        '''
        MapR_-_DO_NOT_TOUCH-v1-policies.json
        providing MapR_-_DO_NOT_TOUCH-v1-policies.json to apicservice and
        comparing with the expected json
        '''
        config_file = 'MapR_-_DO_NOT_TOUCH-v1-policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file, tenant_name="MapR_-_DO_NOT_TOUCH-v1-policies")
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'MapR_-_DO_NOT_TOUCH-v1-policies':
                tenant_existing = json.dumps(tenant.get_json())
                with gzip.open('MapR_-_DO_NOT_TOUCH-v1-policies_tenant_golden.json.gz', 'rb') as data_file:
                    tenant_expected = data_file.read()
                self.assertEqual(DeepDiff(tenant_existing, tenant_expected), {})

    def test_Map_RDO_NOT_TOUCH_v1_policies_with_useipEpgs(self):
        '''
        MapR_-_DO_NOT_TOUCH-v1-policies.json
        providing MapR_-_DO_NOT_TOUCH-v1-policies.json to apicservice and
        comparing with the expected json
        ...
        config_file = 'MapR_-_DO_NOT_TOUCH-v1-policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file, tenant_name="MapR_-_DO_NOT_TOUCH-v1-policies", useipEpgs=True)
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'MapR_-_DO_NOT_TOUCH-v1-policies':
                tenant_existing = json.dumps(tenant.get_json())
                with gzip.open('MapR_-_DO_NOT_TOUCH-v1-policies_with_useipEpgs_tenant_golden.json.gz', 'rb') as data_file:
                    tenant_expected = data_file.read()
                self.assertEqual(DeepDiff(tenant_existing, tenant_expected), {})

    def test_NGINX_DO_NOT_TOUCH_v7_policies(self):
        '''
        NGINX_-_DO_NOT_TOUCH-v7-policies.json
        providing NGINX_-_DO_NOT_TOUCH-v7-policies.json to apicservice and
        comparing with the expected json
        '''
        config_file = 'NGINX_-_DO_NOT_TOUCH-v7-policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file, tenant_name="NGINX_-_DO_NOT_TOUCH-v7-policies")
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'NGINX_-_DO_NOT_TOUCH-v7-policies':
                tenant_existing = json.dumps(tenant.get_json())
                with gzip.open('NGINX_-_DO_NOT_TOUCH-v7-policies_tenant_golden.json.gz', 'rb') as data_file:
                    tenant_expected = data_file.read()
                self.assertEqual(DeepDiff(tenant_existing, tenant_expected), {})

    def test_NGINX_DO_NOT_TOUCH_v7_policies_with_useipEpgs(self):
        '''
        NGINX_-_DO_NOT_TOUCH-v7-policies.json
        providing NGINX_-_DO_NOT_TOUCH-v7-policies.json to apicservice and
        comparing with the expected json
        ...
        config_file = 'NGINX_-_DO_NOT_TOUCH-v7-policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file, tenant_name="NGINX_-_DO_NOT_TOUCH-v7-policies", useipEpgs=True)
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'NGINX_-_DO_NOT_TOUCH-v7-policies':
                tenant_existing = json.dumps(tenant.get_json())
                with gzip.open('NGINX_-_DO_NOT_TOUCH-v7-policies_with_useipEpgs_tenant_golden.json.gz', 'rb') as data_file:
                    tenant_expected = data_file.read()
                self.assertEqual(DeepDiff(tenant_existing, tenant_expected), {})

    def test_Wordpress_DO_NOT_TOUCH_v5_policies(self):
        '''
        Wordpress_-_DO_NOT_TOUCH-v5-policies.json
        providing Wordpress_-_DO_NOT_TOUCH-v5-policies.json to apicservice and
        comparing with the expected json
        '''
        config_file = 'Wordpress_-_DO_NOT_TOUCH-v5-policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file, tenant_name="Wordpress_-_DO_NOT_TOUCH-v5-policies")
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'Wordpress_-_DO_NOT_TOUCH-v5-policies':
                tenant_existing = json.dumps(tenant.get_json())
                with gzip.open('Wordpress_-_DO_NOT_TOUCH-v5-policies_tenant_golden.json.gz', 'rb') as data_file:
                    tenant_expected = data_file.read()
                self.assertEqual(DeepDiff(tenant_existing, tenant_expected), {})

    def test_Wordpress_DO_NOT_TOUCH_v5_policies_with_useipEpgs(self):
        '''
        Wordpress_-_DO_NOT_TOUCH-v5-policies.json
        providing Wordpress_-_DO_NOT_TOUCH-v5-policies.json to apicservice and
        comparing with the expected json
        ...
        config_file = 'Wordpress_-_DO_NOT_TOUCH-v5-policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file, tenant_name="Wordpress_-_DO_NOT_TOUCH-v5-policies", useipEpgs=True)
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'Wordpress_-_DO_NOT_TOUCH-v5-policies':
                tenant_existing = json.dumps(tenant.get_json())
                with gzip.open('Wordpress_-_DO_NOT_TOUCH-v5-policies_with_useipEpgs_tenant_golden.json.gz', 'rb') as data_file:
                    tenant_expected = data_file.read()
                self.assertEqual(DeepDiff(tenant_existing, tenant_expected), {})

    def test_Microsoft_Infrastructure_DO_NOT_TOUCH_v3_policies(self):
        '''
        Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies.json
        providing MapR_-_DO_NOT_TOUCH-v1-policies.json to apicservice and
        comparing with the expected json
        '''
        config_file = 'Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file, tenant_name="Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies")
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies':
                tenant_existing = json.dumps(tenant.get_json())
                with gzip.open('Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies_tenant_golden.json.gz', 'rb') as data_file:
                    tenant_expected = data_file.read()
                self.assertEqual(DeepDiff(tenant_existing, tenant_expected), {})

    def test_Microsoft_Infrastructure_DO_NOT_TOUCH_v3_policies_with_useipEpgs(self):
        '''
        Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies.json
        providing MapR_-_DO_NOT_TOUCH-v1-policies.json to apicservice and
        comparing with the expected json
        ...
        config_file = 'Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(
            config_file,
            tenant_name="Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies",
            useipEpgs=True)
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies':
                tenant_existing = json.dumps(tenant.get_json())
                with gzip.open('Microsoft_Infrastructure_DO_NOT_TOUCH-v3-policies_with_useipEpgs_tenant_golden.json.gz', 'rb') as data_file:
                    tenant_expected = data_file.read()
                self.assertEqual(DeepDiff(tenant_existing, tenant_expected), {})
