...
from acitoolkit import (Tenant,Session,Filter,EPG,Contract,Context,ContractSubject,AppProfile,BridgeDomain,AttributeCriterion)
import sys
import time
...
class LoadConfig(object):
    ...
    def load_configFile(self,config_file,isFile=True,displayonly=False,tenant_name = 'configpush-test',app_name = 'appProfile-test',useipEpgs=False):
        if isFile:
        ...
        self.session = Session('https://'+self.config['apic']['ip_address'], self.config['apic']['user_name'], self.config['apic']['password'])
      ...
      "whitelist": [
        ...
        for tenant in tenants:
            if tenant.name == 'configpush-test':
                ...
                self.assertEquals(len(existing_filters), 2, "filter count didnot match for the pushed config and existing config")
                
                appProfiles = tenant.get_children(AppProfile)
                app = appProfiles[0]
                existing_epgs = app.get_children(EPG)
                self.assertEquals(len(existing_epgs), 2, "epgs count didnot match for the pushed config and existing config")
                for existing_epg in existing_epgs:
                    self.assertEqual(existing_epg._is_attribute_based, False, "attribute based is true for EPG "+existing_epg.name)
                    if existing_epg.name == 'Configpushtest_-_1_-0':
                        self.assertEqual(len(existing_epg.get_all_consumed()),1,"consumed EPG didnot match for EPG "+existing_epg.name)
                        self.assertEqual(len(existing_epg.get_all_provided()),0,"provided EPG didnot match for EPG "+existing_epg.name)
                    elif existing_epg.name == 'Configpushtest_-_2_-1':
                        self.assertEqual(len(existing_epg.get_all_consumed()),0,"consumed EPG didnot match for EPG "+existing_epg.name)
                        self.assertEqual(len(existing_epg.get_all_provided()),1,"provided EPG didnot match for EPG "+existing_epg.name)
                        
                existing_contracts = tenant.get_children(Contract)
                self.assertEquals(len(existing_contracts), 1, "contracts count didnot match for the pushed config and existing config")
                for existing_contract in existing_contracts:
                    for child_contractSubject in existing_contract.get_children(ContractSubject):
                        self.assertEqual(len(child_contractSubject.get_filters()), 2, "num of filters in contractSubject didnot match "+child_contractSubject.name)
                    
                existing_bds = tenant.get_children(BridgeDomain)
                self.assertEquals(len(existing_bds), 0, "bridgeDomains count didnot match for the pushed config and existing config")
                
                existing_contexts = tenant.get_children(Context)
                self.assertEquals(len(existing_contexts), 0, "existing_contexts count didnot match for the pushed config and existing config")
      ...
      "whitelist": [
        ...
        for tenant in tenants:
            if tenant.name == 'configpush-test':
                ...
                self.assertEquals(len(existing_epgs), 4, "epgs count didnot match for the pushed config and existing config")
                for existing_epg in existing_epgs:
                    self.assertEqual(existing_epg._is_attribute_based, False, "attribute based is true for EPG "+existing_epg.name)
                    if existing_epg.name == 'Configpushtest_-_1_-0':
                        self.assertEqual(len(existing_epg.get_all_consumed()),0,"consumed EPG didnot match for EPG "+existing_epg.name)
                ...
                self.assertEquals(len(existing_contracts), 1, "contracts count didnot match for the pushed config and existing config")
                for existing_contract in existing_contracts:
                    self.assertEqual(existing_contract.name, "Configpushtest_-_4_-2::Configpushtest_-_5_-3", "contract name didnot match with the config")
                    self.assertNotEqual(existing_contract.name, "Configpushtest_-_1_-0::Configpushtest_-2_-1", "contract name didnot match with the config")
                    for child_contractSubject in existing_contract.get_children(ContractSubject):
                        self.assertEqual(len(child_contractSubject.get_filters()), 2, "num of filters in contractSubject didnot match "+child_contractSubject.name)
                        self.assertEqual(child_contractSubject.name, "Configpushtest_-_4_-2::Configpushtest_-_5_-3_Subject", "contractSubject name didnot match with the config")
                        self.assertNotEqual(child_contractSubject.name, "Configpushtest_-_1_-0::Configpushtest_-2_-1_Subject", "contractSubject name didnot match with the config")
      ...
      "whitelist": [
        ...
        for tenant in tenants:
            if tenant.name == 'configpush-test':
                ...
                self.assertEquals(len(existing_epgs), 3, "epgs count didnot match for the pushed config and existing config")
                for existing_epg in existing_epgs:
                    if existing_epg.name != 'base':
                        self.assertTrue(existing_epg._is_attribute_based, "uSeg EPG is not created for "+existing_epg.name)
                ...
                existing_bds = tenant.get_children(BridgeDomain)
                self.assertEquals(len(existing_bds), 1, "bridgeDomains count didnot match for the pushed config and existing config")
                
                existing_contexts = tenant.get_children(Context)
                self.assertEquals(len(existing_contexts), 1, "existing_contexts count didnot match for the pushed config and existing config")
                ...
                for existing_epg in existing_epgs:
                    if existing_epg.name != 'base':
                        ...
                        for existing_attributeCriterion in existing_attributeCriterions:
                            self.assertTrue(len(existing_attributeCriterion.get_ip_addresses())==1,"uSeg Attributes didnot match")
...
class TestCheckForAllTheJsonConfigs(unittest.TestCase):
    ...
    def test_configpush_test1_policies(self):
        ...
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file,tenant_name="configpush_test1_policies")
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
    ...
    def test_configpush_test1_policies_with_useipEpgs(self):
        ...
        config_file = 'configpush_test1_policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file,tenant_name="configpush_test1_policies",useipEpgs=True)
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'configpush_test1_policies':
    ...
    def test_configpush_test2_policies(self):
        ...
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file,tenant_name="configpush_test2_policies")
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
    ...
    def test_configpush_test2_policies_with_useipEpgs(self):
        ...
        config_file = 'configpush_test2_policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file,tenant_name="configpush_test2_policies",useipEpgs=True)
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'configpush_test2_policies':
    ...
    def test_configpush_test3_policies(self):
        ...
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file,tenant_name="configpush_test3_policies")
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
    ...
    def test_configpush_test3_policies_with_useipEpgs(self):
        ...
        config_file = 'configpush_test3_policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file,tenant_name="configpush_test3_policies",useipEpgs=True)
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'configpush_test3_policies':
    ...
    def test_configpush_test4_policies(self):
        ...
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file,tenant_name="configpush_test4_policies")
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
    ...
    def test_configpush_test4_policies_with_useipEpgs(self):
        ...
        config_file = 'configpush_test4_policies.json.gz'
        loadConfig = LoadConfig()
        loadConfig.load_configFile(config_file,tenant_name="configpush_test4_policies",useipEpgs=True)
        time.sleep(5)
        tenants = Tenant.get_deep(loadConfig.session, names=[loadConfig.tool._tenant_name])
        for tenant in tenants:
            if tenant.name == 'configpush_test4_policies':
